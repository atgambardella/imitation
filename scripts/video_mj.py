...
def main(policy, env_name, max_steps=301, deterministic=1):
    ...
    policy_file, policy_key = util.split_h5_name(policy)
    ...
    util.header('Max steps is {}'.format(max_steps))
    ...
    while not exit:
        ...
        #"""
        while not sim.done:
            a = policy.sample_actions(sim.obs[None,:], bool(deterministic))[0][0,:]
            sim.step(a)
            sim.render()
            """
            #sim.draw()
